...
function createSimilarSets(targetNumElements, targetJaccard, seed = 1) {
    ...
    let valCounter = seed * 43; // Offset by seed to get different elements for different seeds
...
test('Approximate Similarity - Early Exit Behavior', async () => {
    const numHashes = 128;
    const numGroups = 4;
    const bitDepth = 32;
    ...
    assert.ok(result1 > opts1.similarityThreshold, `Threshold ${opts1.similarityThreshold} (far below actual J ~${actualJ.toFixed(2)}): Should return similarity > threshold`);
    assert.ok(result1 >= 0 && result1 <= 1, `Result should be a valid Jaccard similarity value`);
    assert.notEqual(result1, 1.0, `Should return estimated similarity, not 1.0`);
    ...
    assert.strictEqual(result2, 0.0, `Threshold ${opts2.similarityThreshold} (far above actual J ~${actualJ.toFixed(2)}): Should return 0.0 (confidently dissimilar)`);
    ...
    assert.ok(result3 >= 0 && result3 <= 1, `Threshold ${opts3.similarityThreshold} (near actual J ~${actualJ.toFixed(2)}): Should return a Jaccard value`);
    assert.strictEqual(result3, fullComparison, `Threshold ${opts3.similarityThreshold}: Result should match full comparison if no early exit`);
    ...
    assert.ok(result4 >= 0 && result4 <= 1.0, `Threshold ${opts4.similarityThreshold} (moderately below actual J ~${actualJ.toFixed(2)}) with high epsilon: Should return valid Jaccard value`);
    if (result4 !== 0.0) { // If not confidently dissimilar
        assert.ok(result4 >= opts4.similarityThreshold, `If not dissimilar, result should be >= threshold`);
    }
    ...
    assert.ok(result5 === 0.0 || (result5 >= 0 && result5 <= 1.0), `Threshold ${opts5.similarityThreshold} (moderately above actual J ~${actualJ.toFixed(2)}) with high epsilon: May return 0.0 or actual Jaccard`);
});

test('Approximate Similarity - Statistical Accuracy (Monte Carlo)', async () => {
    const numHashes = 128;
    const numGroups = 4;
    const bitDepth = 32;
    const iterations = MAX_ITERS; // Number of Monte Carlo iterations (reduced from 100000)
    ...
    assert.ok(errorRateA <= epsilonA + 0.02, `Error rate for 'confidently similar' (T=${thresholdA}, J_act~${targetJaccard}, eps=${epsilonA}) should be around ${epsilonA * 100}%. Got: ${(errorRateA * 100).toFixed(2)}% (errors: ${errorsA})`);
    ...
    assert.ok(errorRateB <= epsilonB + 0.02, `Error rate for 'confidently dissimilar' (T=${thresholdB}, J_act~${targetJaccard}, eps=${epsilonB}) should be around ${epsilonB * 100}%. Got: ${(errorRateB * 100).toFixed(2)}% (errors: ${errorsB})`);

    // Monte Carlo tests are statistical; minor variations are expected. An additional margin (+2%) is added to epsilon for the assertion.
});

test('Approximate Similarity - Statistical Accuracy (Monte Carlo) - AGGRESSIVE EPSILON', async () => {
    const numHashes = 128;
    const numGroups = 4;
    const bitDepth = 32;
    const iterations = MAX_ITERS; // Number of Monte Carlo iterations
    const targetJaccard = 0.5;
    const setSize = 200;
    ...
    assert.ok(errorRateAggressiveA <= aggressiveEpsilon + 0.03, `AGGRESSIVE Error rate for 'confidently similar' (T=${thresholdA}, J_act~${targetJaccard}, aggressive_eps=${aggressiveEpsilon}) should be around ${aggressiveEpsilon * 100}%. Got: ${(errorRateAggressiveA * 100).toFixed(2)}% (errors: ${errorsAggressiveA})`);
    ...
    assert.ok(errorRateAggressiveB <= aggressiveEpsilon + 0.20, `AGGRESSIVE Error rate for 'confidently dissimilar' (T=${thresholdB}, J_act~${targetJaccard}, aggressive_eps=${aggressiveEpsilon}) should be around ${aggressiveEpsilon * 100}%. Got: ${(errorRateAggressiveB * 100).toFixed(2)}% (errors: ${errorsAggressiveB})`);

    console.log('Aggressive Monte Carlo tests use a higher epsilon. A larger margin (+20%) is added for statistical variance and estimation uncertainty.');
});

test('Approximate Similarity - Input Validations for Options', async () => {
    ...
    }, { message: /Invalid or missing 'numGroups' for optimized similarity estimation/ }, 'Throws if numGroups is missing and options are provided');

    assert.throws(() => {
        estimateJaccardSimilarity(sig, sig, { numGroups: 2, errorTolerance: 0.01 });
    }, { message: /Invalid or missing 'similarityThreshold' for optimized similarity estimation/ }, 'Throws if similarityThreshold is missing');

    assert.throws(() => {
        estimateJaccardSimilarity(sig, sig, { numGroups: 2, similarityThreshold: 0.5 });
    }, { message: /Invalid or missing 'errorTolerance' for optimized similarity estimation/ }, 'Throws if errorTolerance is missing');

    assert.throws(() => {
        estimateJaccardSimilarity(sig, sig, { numGroups: 0, similarityThreshold: 0.5, errorTolerance: 0.01 });
    }, { message: /Invalid or missing 'numGroups' for optimized similarity estimation/ }, 'Throws if numGroups is 0');

    assert.throws(() => {
        estimateJaccardSimilarity(sig, sig, { numGroups: 3, similarityThreshold: 0.5, errorTolerance: 0.01 });
    }, { message: /Invalid or missing 'numGroups' for optimized similarity estimation.*divisor of signature length/ }, 'Throws if signature length (8) is not divisible by numGroups (3)');
    ...
    assert.strictEqual(resNoOptions, 1.0, 'If no optimization options are provided, defaults to full comparison and does not throw.');

    const resUndefinedOptions = estimateJaccardSimilarity(sig, sig, undefined);
    assert.strictEqual(resUndefinedOptions, 1.0, 'If options object is undefined, defaults to full comparison and does not throw.');
    ...
    assert.strictEqual(resNullOptions, 1.0, 'If options object is null, defaults to full comparison (as it becomes {}).');

    console.log('Validation tests now expect specific errors for incomplete/invalid optimization options.');
});

test('Fast Approximation Accuracy (Monte Carlo) - maxGroups Analysis', async () => {
    const numHashes = 128;
    const numGroups = 4;
    const bitDepth = 32;
    const iterations = MAX_ITERS; // Sufficient for statistical analysis
    const setSize = 200;
    ...
    for (const targetJaccard of similarityLevels) {
        ...
        assert.ok(avgError1 <= 0.15, `1 group: Average error vs 4 groups should be ≤0.15 for J~${targetJaccard}. Got: ${avgError1.toFixed(4)}`);
        assert.ok(avgError2 <= 0.08, `2 groups: Average error vs 4 groups should be ≤0.08 for J~${targetJaccard}. Got: ${avgError2.toFixed(4)}`);
        assert.ok(avgError3 <= 0.04, `3 groups: Average error vs 4 groups should be ≤0.04 for J~${targetJaccard}. Got: ${avgError3.toFixed(4)}`);
        
        assert.ok(maxError1 <= 0.5, `1 group: Max error vs 4 groups should be ≤0.5 for J~${targetJaccard}. Got: ${maxError1.toFixed(4)}`);
        assert.ok(maxError2 <= 0.3, `2 groups: Max error vs 4 groups should be ≤0.3 for J~${targetJaccard}. Got: ${maxError2.toFixed(4)}`);
        assert.ok(maxError3 <= 0.2, `3 groups: Max error vs 4 groups should be ≤0.2 for J~${targetJaccard}. Got: ${maxError3.toFixed(4)}`);
        
        // Verify that more groups generally give better accuracy
        assert.ok(avgError1 >= avgError2, `1 group should be less accurate than 2 groups for J~${targetJaccard}`);
        assert.ok(avgError2 >= avgError3, `2 groups should be less accurate than 3 groups for J~${targetJaccard}`);
        
        console.log(`Similarity ${targetJaccard}: Avg errors - 1g:${avgError1.toFixed(4)}, 2g:${avgError2.toFixed(4)}, 3g:${avgError3.toFixed(4)}`);
    }
    
    console.log('Fast approximation maintains reasonable accuracy while providing significant speed improvements.');
});

test('Fast Approximation Performance vs Accuracy Trade-off Analysis', async () => {
    const numHashes = 128;
    const numGroups = 4;
    const bitDepth = 32;
    const iterations = MAX_ITERS;
    const setSize = 150;
    ...
    const tolerance = 0.5; // 20% tolerance for timing variance (increased from 10%)
    ...
    assert.ok(speedup1vs4 >= 1.1, `1 group should be ≥1.1x faster than 4 groups. Got: ${speedup1vs4.toFixed(2)}x`);
    assert.ok(speedup2vs4 >= 1.1, `2 groups should be ≥1.1x faster than 4 groups. Got: ${speedup2vs4.toFixed(2)}x`);
    
    // All results should be valid similarities
    for (let groups = 1; groups <= 4; groups++) {
        assert.ok(avgSimilarities[groups] >= 0 && avgSimilarities[groups] <= 1, 
             `${groups} groups: Average similarity should be valid (0-1). Got: ${avgSimilarities[groups].toFixed(4)}`);
    }
    
    console.log(`Speed improvements: 1g=${speedup1vs4.toFixed(2)}x, 2g=${speedup2vs4.toFixed(2)}x faster than 4g`);
    console.log(`Average similarities: 1g=${avgSimilarities[1].toFixed(4)}, 2g=${avgSimilarities[2].toFixed(4)}, 3g=${avgSimilarities[3].toFixed(4)}, 4g=${avgSimilarities[4].toFixed(4)}`);
    console.log(`Average times: 1g=${avgTimes[1].toFixed(4)}ms, 2g=${avgTimes[2].toFixed(4)}ms, 3g=${avgTimes[3].toFixed(4)}ms, 4g=${avgTimes[4].toFixed(4)}ms`);
});

test('Fast Approximation Input Validation', async () => {
    ...
    assert.ok(fastResult >= 0 && fastResult <= 1, 'Fast mode should work without threshold/errorTolerance');
}); 